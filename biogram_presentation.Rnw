...
\item Sparser encoding requires larger data sets.~\cite{2002linamino}.
\end{itemize}
\end{frame}

\begin{frame}
Clustering of amini acids using a set of physicochemical properties specific for a given problem removes redundant information and simplifies model.
\end{frame}
...
Encoding distance between two encodings: 8.
